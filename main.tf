// Create VPC
module "vpc" {
  source    = "github.com/silinternational/terraform-modules//aws/vpc-public-only?ref=2.7.1"
...
// Create ecs cluster
module "ecscluster" {
  source   = "github.com/silinternational/terraform-modules//aws/ecs/cluster?ref=2.7.1"
  app_name = "${var.app_name}"
  app_env  = "${var.app_env}"
}
...
// Create DB
module "rds" {
  source                  = "github.com/silinternational/terraform-modules//aws/rds/mariadb?ref=2.7.1"
  app_name                = "${var.app_name}"
  app_env                 = "${var.app_env}"
...
module "alb" {
  source          = "github.com/silinternational/terraform-modules//aws/alb?ref=2.7.1"
  app_name        = "${var.app_name}"
  app_env         = "${var.app_env}"
  internal        = "false"
  vpc_id          = "${module.vpc.id}"
...
module "ecsservice_buildengine" {
  source             = "github.com/silinternational/terraform-modules//aws/ecs/service-only?ref=2.7.1"
...
module "cloudflare-sg" {
  source = "github.com/silinternational/terraform-modules//aws/cloudflare-sg?ref=2.7.1"
  vpc_id = "${module.vpc.id}"
}
